...
exports.getFinishedGoods = async (req, res, next) => {
  try {
    ...
    console.log(error);
    res.send({
      message: "Could not get list of finished goods. Please reload the page.",
    });
  }
};
...
exports.updateFinishedGoods = async (req, res) => {
  try {
    ...
    const result = await FinishedGoods.findOneAndUpdate(
      { productCode: req.body.productCode },
    ...
    console.log(error);
    res.send({
      message: "Something went wrong while updating finished goods.",
    });
  }
};
...
exports.dispatchFinishedGoods = async (req, res, next) => {
  try {
    for (const key in req.body.items) {
      const result = await FinishedGoods.findOneAndUpdate(
        { productCode: key },
        { $inc: { quantity: -1 * req.body.items[key] } }
      );
    }
    next();
  } catch (error) {
